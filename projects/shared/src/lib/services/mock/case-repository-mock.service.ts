...
} from '@shared/models/shared.enums';
import {
...
export class CaseRepositoryMockService implements CaseRepositoryService {
  private cases: CaseSummaryDto[] = MOCK_CASE_SUMMARIES;
  ...
  getAll(): Observable<CaseSummaryDto[]> {
    ...
    const updatedCases = this.cases.map((c) => {
      ...
      const randomUpdateTime = dayjs(clone.creationDate)
        .add(1, 'minute')
        .add(Math.floor(Math.random() * 4), 'minute')
        .toDate()
        .getTime();
    ...
    return of(updatedCases.sort((a, b) => b.creationDate.getTime() - a.creationDate.getTime()));
  }
